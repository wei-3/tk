<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:86:"D:\phpStudy\WWW\twothink\public/../application/admin/view/default/property\active.html";i:1507184851;s:82:"D:\phpStudy\WWW\twothink\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<body>
	...
	<div class="cf">
		<a class="btn" href="<?php echo url('active_add'); ?>">新 增</a>
		<button class="btn ajax-post confirm" url="<?php echo url('active_del'); ?>" target-form="ids">删 除</button>
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="name" class="search-input" value="<?php echo input('name'); ?>" placeholder="请输入报修人">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo url('active'); ?>"><i class="btn-search"></i></a>
			</div>
		</div>
	</div>
		...
		<table>
			...
			<tbody>
				<?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$active): $mod = ($i % 2 );++$i;?>
					<tr>
						<td><input class="ids row-selected" type="checkbox" name="id[]" id="" value="<?php echo $active['id']; ?>"> </td>
						<td><?php echo $active['name']; ?></td>
						<td><?php echo $active['active_name']; ?></td>
						<td><?php echo $active['active_intro']; ?></td>
						<td><?php echo $active['application_time']; ?></td>
                        <td><?php echo $active['end_time']; ?></td>
                        <td><?php echo $active['status_text']; ?></td>
						<td>
							<a title="编辑" href="<?php echo url('active_edit?id='.$active['id']); ?>">编辑</a>
							<a href="<?php echo url('setStatus?ids='.$active['id'].'&status='.abs(1-$active['status'])); ?>" class="ajax-get"><?php echo show_handle_op($active['status']); ?></a>
							<a class="confirm ajax-get" title="删除" href="<?php echo url('active_del?id='.$active['id']); ?>">删除</a>
						</td>
					</tr>
