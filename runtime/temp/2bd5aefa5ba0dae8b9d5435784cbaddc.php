<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:81:"D:\phpStudy\WWW\twothink\public/../application/admin/view/default/sale\index.html";i:1507347579;s:82:"D:\phpStudy\WWW\twothink\public/../application/admin/view/default/public\base.html";i:1496373782;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
...
<body>
	...
	<div class="cf">
		...
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="name" class="search-input" value="<?php echo input('title'); ?>" placeholder="请输入标题">
		...
		<table>
			...
			<tbody>
				<?php if(!(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty()))): if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sale): $mod = ($i % 2 );++$i;?>
					<tr>
						<td><input class="ids row-selected" type="checkbox" name="id[]" id="" value="<?php echo $sale['id']; ?>"> </td>
						<td><?php echo $sale['name']; ?></td>
						<td><?php echo $sale['tel']; ?></td>
						<td><?php echo $sale['title']; ?></td>
						<td><?php echo $sale['type']; ?></td>
						<td><?php echo $sale['price']; ?><?php echo $sale['unit']; ?></td>
                        <td><?php echo time_format($sale['create_time']); ?></td>
                        <td><?php echo $sale['end_time']; ?></td>
                        <td><?php echo $sale['status_text']; ?></td>
						<td>
							<a title="编辑" href="<?php echo url('edit?id='.$sale['id']); ?>">编辑</a>
							<a href="<?php echo url('setStatus?ids='.$sale['id'].'&status='.abs(1-$sale['status'])); ?>" class="ajax-get"><?php echo show_handle_op($sale['status']); ?></a>
